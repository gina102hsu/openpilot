...
const AddrBus TOYOTA_TX_MSGS[] = {{0x283, 0}, {0x2E6, 0}, {0x2E7, 0}, {0x33E, 0}, {0x344, 0}, {0x365, 0}, {0x366, 0}, {0x4CB, 0},  // DSU bus 0
                                  ...
                                  {0x200, 0}, {0x750, 0}, {0x5AA, 0}};  // interceptor + Blindspot monitor
...
static int toyota_tx_hook(CAN_FIFOMailBox_TypeDef *to_send) {
  ...
  if (bus == 0) {
    ...
    if (addr == 0x5aa)
    {
       ...
       {
          eon_counter++;
          if (eon_counter>5)   // 5*50ms, 250ms. prevent switch between stock and eon too freqently 
          {
    ...
    if (addr == 0x191)
    {
       ...
       {
          uint8_t chksum;
          to_send->RDLR&=0xFDFFFFFEU;
          chksum=toyota_compute_checksum(to_send);
          to_send->RDHR&=0x00FFFFFFU;
          to_send->RDHR|=((chksum<<24)&0xFF000000U);
       }
